...
export const ExperienceTemplate = ({ ...props }: ExperienceType) => {
  const t = useTranslations("Experience");
  const currentLocale = useLocale() as LanguageCode;

  return (
    <section className="z-20 flex w-full flex-col items-center">
      <div className="mt-6 mb-4 flex w-full flex-col gap-4 self-start px-1 sm:px-0">
        <TextAnimate
          className={cn(
        ...
        <TextAnimate
          className="text-darkest/90 max-w-[800px] text-sm font-semibold text-balance dark:text-acc-yellow-3"
          animation="slideUp"
          as="h2"
          by="line"
          once
        ...
        <ExperienceLink
          className={cn(
            "text-darkest/90 mb-4 text-base font-semibold",
            "hover:text-acc-red",
            "dark:text-acc-yellow/90 dark:hover:text-acc-yellow-2",
          )}
          title={t("view")}
          url={props.repoLink}
        />
      ...
      >
        <div
          className={cn(
            "col-span-2 mb-10 flex flex-col items-center justify-between gap-8 p-1 sm:p-0",
            "xl:mb-0",
          )}
        >
          <div>
            {props.body[currentLocale].map((line, index) => (
              <TextAnimate
                key={index}
                className={cn(
                  "text-darkest/95 font-crimson mb-3 text-xl text-pretty last:mb-0 last:pb-0 dark:text-off-w/85 xl:text-2xl",
                )}
                animation="slideLeft"
                delay={0.15}
                duration={0.1}
          ...
          <div
            className={cn(
              "flex flex-wrap items-center justify-center gap-5 self-start p-2",
            )}
          >
